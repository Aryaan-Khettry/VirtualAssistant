...
def assistant_speaks(output):
    ...
    file=str(inc)+".mp3"
    toSpeak.save(file)
    playsound.playsound(file, True) #play the audio in the file
    os.remove(file) #remove file after speech

def get_audio():
  
    rObject = sr.Recognizer()
    audio=''
    with sr.Microphone() as source: #Take user speech as input
        print("Speak..")
          
        audio = rObject.listen(source, phrase_time_limit = 10) #user speech stored in audio
    print("Processing...") #Waits for 10 seconds 
  
    try:
  
        text = rObject.recognize_google(audio, language ='en-US') #Used to parse the users speech and place it in an array
        print(name + " : ", text)
        return text
